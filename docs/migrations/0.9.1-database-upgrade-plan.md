...
根據現有的 `shoppingcar-service-0.9.0.sql` 資料庫結構和重構計劃，我建議採用漸進式升級策略，保留原有資料表並逐步擴充新功能，同時確保系統正常運行。

## 升級策略

1. **保留原有表格**：現有的表格結構和資料將被保留
2. **擴展現有表格**：為現有表格添加必要的新欄位
3. **創建新表格**：為模組化設計添加新的詳情表和統計表
4. **建立表格關聯**：確保所有表格間的關聯正確

## 資料庫升級 SQL (shoppingcar-service-0.9.1.sql)
...
### 1. 用戶模組擴展
- 擴展 `users` 表，添加電子郵件、會員等級和狀態欄位
- 創建 `user_details` 表儲存更詳細的用戶信息

### 2. 商家模組擴展
- 擴展 `merchants` 表，添加類別和狀態欄位
- 創建多個子表分別儲存商家的詳細信息、聯絡人、支付信息和媒體資源

### 3. 產品模組擴展
- 擴展 `products` 表，添加SKU、促銷價和狀態欄位
- 重命名 `stock_amount` 為 `stock` 以符合新設計
- 創建產品詳情、SEO和圖片表

### 4. 訂單模組擴展
- 擴展 `orders` 表，添加訂單狀態和支付狀態欄位
- 創建訂單詳情和配送表
- 擴展 `order_items` 表並重命名數量欄位

### 5. 購物車模組擴展
- 重命名購物車項目表中的數量欄位

### 6. 統計模組創建
- 創建三個新表來支持儀表板功能：儀表板統計表、銷售趨勢表和產品熱門度表
...
1. **無縫升級**：保留現有表格結構，可以在不中斷服務的情況下進行升級
2. **數據完整性**：確保所有現有數據都被正確遷移
3. **向後兼容**：在升級過程中保持原有API的功能
4. **模組化設計**：遵循提案的模組化設計，使系統更易於維護
...
此升級方案將確保系統從0.9.0順利過渡到0.9.1版本，同時保留所有現有功能並添加新的能力。