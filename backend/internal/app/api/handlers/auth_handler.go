...
// @Produce json
// @Security Bearer
// @Success 200 {object} interfaces.UserResponse "用戶信息"
// @Failure 401 {object} interfaces.ErrorResponse "未授權"
// @Router /api/v1/auth/token [post]
func (h *AuthHandler) ValidateToken(c *gin.Context) {
	userID, exists := c.Get("userID")
	if !exists {
		c.JSON(http.StatusUnauthorized, interfaces.ErrorResponse{Error: "未認證"})
		return
	}

	user, err := h.userService.GetUserById(userID.(uint))
	if err != nil {
		c.JSON(http.StatusInternalServerError, interfaces.ErrorResponse{Error: "無法獲取用戶信息"})
		return
	}
